...
public partial class Controls_LoginControl : System.Web.UI.UserControl
{
    ...
    protected void btnLogin_Click(object sender, EventArgs e)
    {
        try
        {
           ...
           // string loginPwd = MD5(txtLoginPwd.Text.Trim(), 32);
            ...
            {
                getLoginOk(loginId, errId.ToString());      //调用getLoginOk方法实现用户登录
            }
            else
            {
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        Response.Write("<script language='javascript'>location.href='UserLogin.aspx';</script>");
        Response.Cookies["count"].Value = "3";      //请求响应写入临时cookie["count"]初始值
        Response.Cookies["count"].Expires = DateTime.Now.AddMinutes(15);
    ...
    private void GetHttpCookie()
    {
        ...
        else
        {
            ...
            //this.lblIP.Text = Request.ServerVariables["REMOTE_ADDR"].ToString();
    ...
    private void getLoginOk(string loginId,string errId)
    {
        ...
        if (cbTime.Checked)
        {
            cookieLogin.Expires = DateTime.Now.AddDays(14);
        }
        else
        {
            cookieLogin.Expires = DateTime.Now.AddMinutes(60);
        }
        Response.Cookies.Add(cookieLogin);

        Response.Write("<script language='javascript'>alert('登录成功，正在跳转首页！');location.href='../Default.aspx';</script>");
    }
